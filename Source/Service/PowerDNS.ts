...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	protected async clientRequest($clientId: string, $stream: net.Socket, $data: Buffer): Promise<void> {
		...
		try {
			// Parse the payload
			let $payload: {method: string, parameters: ({zonename: string, domain_id?: number}|{qtype: string, qname: string, remote: string, local: string, 'real-remote': string, 'zone-id': number})} = JSON.parse($data.toString());
			// Log the payload
			this.logger().debug($utility.util.format('Client [%s] Payload\t%s', $clientId, JSON.stringify($payload)));
			// Instantiate our model
			let $queryModel: DnsQuery = await DnsQuery.create({
				clientId: $clientId,
				method: $payload.method,
				request: $payload
			});
			// Instantiate the library
			let $libPowerDNS: PowerDNS = new PowerDNS($queryModel, this.logger());
			// Await the response
			...
			let $powerDnsResult: PowerDnsResult = new PowerDnsResult();
